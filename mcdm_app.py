...
def weight_method_groups(layer_key: str = "all") -> List[tuple[str, List[str]]]:
    groups = [
        (
            tt("Bilgi ve dağılım temelli", "Information and dispersion based"),
            ["Entropy", "Fuzzy Entropy", "Standart Sapma", "Fuzzy Standart Sapma", "LOPCOW", "Fuzzy LOPCOW"],
        ),
        (
            tt("İlişki ve yapı temelli", "Relation and structure based"),
            ["CRITIC", "Fuzzy CRITIC", "PCA", "Fuzzy PCA"],
        ),
        (
            tt("Etki ve hibrit temelli", "Impact and hybrid based"),
            ["MEREC", "Fuzzy MEREC", "CILOS", "Fuzzy CILOS", "IDOCRIW", "Fuzzy IDOCRIW"],
        ),
        (
    ...
    if layer_key == "classical":
        return [(label, [m for m in methods if not m.startswith("Fuzzy ")]) for label, methods in groups]
    if layer_key == "fuzzy":
        return [(label, [m for m in methods if m.startswith("Fuzzy ")]) for label, methods in groups]
    return groups
...
def _render_weight_method_selector(method_groups: List[tuple[str, List[str]]], methods_internal: List[str]) -> None:
    for group_label, group_methods in method_groups:
        ...
        for i, method_name in enumerate(_filtered_methods):
            with method_cols[i % len(method_cols)]:
                ...
                st.checkbox(
                    ...
                    key=_cb_key,
                    help=_w_help,
                    on_change=_wm_single_select_cb,
                    args=(method_name, methods_internal),
                )
...
def _criterion_input_columns(criteria_cols: List[str], fuzzy_tfn: bool = False) -> List[str]:
    if not fuzzy_tfn:
        return list(criteria_cols)
    out: List[str] = []
    for crit in criteria_cols:
        out.extend([f"{crit}_l", f"{crit}_m", f"{crit}_u"])
    return out


def _fuzzify_sample_dataset(df: pd.DataFrame, protected_cols: List[str]) -> pd.DataFrame:
    ...
    for col in df.columns:
        ...
        if numeric.isna().all():
            out[col] = series
            continue
        middle = numeric.astype(float)
        spread = rng.uniform(0.06, 0.16, len(middle))
        width = np.maximum(np.abs(middle) * spread, 0.05)
        lower = np.round(middle - width, 4)
        upper = np.round(middle + width, 4)
        out[f"{col}_l"] = lower
        out[f"{col}_m"] = np.round(middle, 4)
        out[f"{col}_u"] = upper
    return out


def sample_fuzzy_dataset() -> pd.DataFrame:
...
def sample_fuzzy_dataset_en() -> pd.DataFrame:
...
def sample_panel_fuzzy_dataset_en() -> pd.DataFrame:
    return _fuzzify_sample_dataset(sample_panel_dataset_en(), ["Country", "Year"])


def _parse_tfn_column_name(column_name: str) -> tuple[str, str] | None:
    text = str(column_name or "").strip()
    match = re.match(r"^(.*?)(?:[\s_.-]+|\()([lmuLMU])\)?$", text)
    if not match:
        return None
    base = str(match.group(1) or "").strip()
    part = str(match.group(2) or "").strip().lower()
    if not base or part not in {"l", "m", "u"}:
        return None
    return base, part


def _detect_tfn_groups(columns: List[str]) -> Dict[str, Dict[str, str]]:
    groups: Dict[str, Dict[str, str]] = {}
    for col in columns:
        parsed = _parse_tfn_column_name(str(col))
        if not parsed:
            continue
        base, part = parsed
        groups.setdefault(base, {})[part] = str(col)
    return {base: parts for base, parts in groups.items() if {"l", "m", "u"}.issubset(parts.keys())}


def _preprocess_tfn_dataset(df: pd.DataFrame) -> tuple[pd.DataFrame, Dict[str, Any]]:
    groups = _detect_tfn_groups(list(df.columns))
    if not groups:
        raise ValueError(
            tt(
                "Üçgensel bulanık veri seçildi ancak `_l`, `_m`, `_u` sütun üçlüleri bulunamadı.",
                "Triangular fuzzy input is selected but no `_l`, `_m`, `_u` column triplets were found.",
            )
        )

    out = pd.DataFrame(index=df.index)
    used_cols = {col for parts in groups.values() for col in parts.values()}
    for col in df.columns:
    ...
    for base, parts in groups.items():
        lower = pd.to_numeric(df[parts["l"]], errors="coerce")
        middle = pd.to_numeric(df[parts["m"]], errors="coerce")
        upper = pd.to_numeric(df[parts["u"]], errors="coerce")
        valid_mask = lower.notna() | middle.notna() | upper.notna()
        if valid_mask.any():
            bad_order = ((lower > middle) | (middle > upper)) & valid_mask
            if bool(bad_order.any()):
                raise ValueError(
                    tt(
                        f"`{base}` kriteri için bazı satırlarda l ≤ m ≤ u koşulu sağlanmıyor.",
                        f"For criterion `{base}`, some rows do not satisfy l ≤ m ≤ u.",
                    )
                )
        out[base] = (lower + middle + upper) / 3.0
        local_spread = ((upper - lower).abs() / (2.0 * middle.abs().replace(0.0, np.nan))).replace([np.inf, -np.inf], np.nan)
        local_mean = float(local_spread.dropna().mean()) if not local_spread.dropna().empty else np.nan
        if np.isfinite(local_mean):
            spread_vals.append(local_mean)
        detail_rows.append(
            {
                "Criterion": base,
                "LowerCol": parts["l"],
...
def _preprocess_input_dataset(df: pd.DataFrame, value_mode: str) -> tuple[pd.DataFrame, Dict[str, Any]]:
    mode = str(value_mode or "crisp").strip().lower()
    if mode == "tfn":
...
def _seed_manual_entry_df(
    ...
    fuzzy_tfn: bool = False,
) -> pd.DataFrame:
    row_count = int(max(2, row_count))
    value_cols = _criterion_input_columns(criteria_cols, fuzzy_tfn=fuzzy_tfn)
    all_cols = [entity_col] + ([year_col] if year_col else []) + value_cols
...
def _manual_schema_columns(entity_col: str, criteria_cols: List[str], year_col: str | None = None, fuzzy_tfn: bool = False) -> List[str]:
    return [entity_col] + ([year_col] if year_col else []) + _criterion_input_columns(criteria_cols, fuzzy_tfn=fuzzy_tfn)
...
def _parse_manual_paste_text(
    ...
    *,
    ...
    expected_cols = _manual_schema_columns(entity_col, criteria_cols, year_col, fuzzy_tfn=fuzzy_tfn)
...
def _prepare_manual_entry_df(
    ...
    out = edited_df.copy()
    value_cols = _criterion_input_columns(criteria_cols, fuzzy_tfn=fuzzy_tfn)
    missing_cols = [col for col in [entity_col] + ([year_col] if year_col else []) + value_cols if col not in out.columns]
    if missing_cols:
        raise ValueError(
            tt(
    ...
    out = out[[entity_col] + ([year_col] if year_col else []) + value_cols].copy()
    out[entity_col] = out[entity_col].astype(str).replace({"nan": "", "None": ""}).str.strip()
    numeric_targets = value_cols + ([year_col] if year_col else [])
    for col in numeric_targets:
        out[col] = pd.to_numeric(out[col], errors="coerce")
    keep_mask = out[entity_col].ne("") | out[value_cols].notna().any(axis=1)
    if year_col:
    ...
    if not out[value_cols].notna().any().any():
        raise ValueError(
            tt(
...
def _activate_data_source(df: pd.DataFrame, source_id: str, entry_mode: str) -> None:
    value_mode = str(st.session_state.get("data_value_mode", "crisp") or "crisp")
    processed_df, input_profile = _preprocess_input_dataset(df, value_mode)
    st.session_state["raw_data_original"] = df.copy()
    st.session_state["raw_data"] = processed_df.copy()
    ...
    st.session_state["analysis_scope"] = "panel" if _guess_year_columns(processed_df) else "single"
    access.track_event(
        "data_source_loaded",
        {
...
def _apply_manual_preset(preset: Dict[str, Any], lang: str) -> None:
    ...
    st.session_state["manual_entry_df"] = _seed_manual_entry_df(
        ...
        fuzzy_tfn=str(st.session_state.get("data_value_mode", "crisp")) == "tfn",
    )
...
def _fill_manual_demo_data(
    ...
    for idx, col in enumerate(criteria_cols, start=1):
        ...
        middle = np.round(base + trend, 2)
        if fuzzy_tfn:
            width = np.maximum(np.abs(middle) * (0.08 + idx * 0.01), 0.05)
            out[f"{col}_l"] = np.round(middle - width, 2)
            out[f"{col}_m"] = middle
            out[f"{col}_u"] = np.round(middle + width, 2)
...
def _manual_editor_stats(
    ...
    value_cols = _criterion_input_columns(criteria_cols, fuzzy_tfn=fuzzy_tfn)
    numeric_df = df[value_cols].apply(pd.to_numeric, errors="coerce") if value_cols else pd.DataFrame(index=df.index)
    ...
    total_numeric_slots = int(max(1, len(value_cols) * max(filled_rows, 1)))
...
def _render_upload_data_source_section(lang: str) -> None:
    ...
    _is_tfn_input = str(st.session_state.get("data_value_mode", "crisp")) == "tfn"
    st.caption(
        tt(
            "Beklenen format: ilk sütun alternatif etiketi olabilir. Kesin modda kriter sütunları sayısal olmalıdır; TFN modunda her kriter için `_l`, `_m`, `_u` sütun üçlüsü beklenir. CSV, XLSX ve SPSS `.sav` desteklenir.",
            "Expected format: the first column may contain alternative labels. In crisp mode, criterion columns must be numeric; in TFN mode, each criterion is expected as an `_l`, `_m`, `_u` triplet. CSV, XLSX, and SPSS `.sav` are supported.",
        )
    )
    st.caption(
        tt(
    ...
    if _show_tpl:
        render_table(_input_format_notes(lang, panel=False, fuzzy_tfn=_is_tfn_input))
        st.markdown(f"**{tt('Tek dönem örnek görünüm', 'Single-period example preview')}**")
        _sample_preview = sample_fuzzy_dataset().head(5) if (_is_tfn_input and lang != "EN") else (
            sample_fuzzy_dataset_en().head(5) if _is_tfn_input else (sample_dataset().head(5) if lang != "EN" else sample_dataset_en().head(5))
        )
        render_table(_sample_preview)
        ...
        with _tpl_col1:
            _single_tpl_clicked = st.download_button(
                ...
                data=generate_input_template_excel(lang=lang, panel=False, fuzzy_tfn=_is_tfn_input),
    ...
    with sample_col_1:
        ...
        if st.button(_sample_tr_label, use_container_width=True, key="btn_sample_tr_main"):
            _stage_data_source(sample_fuzzy_dataset() if _is_tfn_input else sample_dataset(), "sample_data_tr")
            st.rerun()
    with sample_col_2:
        _sample_en_label = tt("📗 Örnek Fuzzy Veri (EN)", "📗 Sample Fuzzy Data (EN)") if _is_tfn_input else tt("📗 Örnek Veri (EN)", "📗 Sample Data (EN)")
        if st.button(_sample_en_label, use_container_width=True, key="btn_sample_en_main"):
            _stage_data_source(sample_fuzzy_dataset_en() if _is_tfn_input else sample_dataset_en(), "sample_data_en")
            st.rerun()
    with sample_col_3:
        _panel_sample_label = tt("📙 Fuzzy Panel Veri (EN)", "📙 Fuzzy Panel Data (EN)") if _is_tfn_input else tt("📙 Panel Veri (EN)", "📙 Panel Data (EN)")
        if st.button(_panel_sample_label, use_container_width=True, key="btn_sample_panel_en_main"):
    ...
    if isinstance(_pending_df, pd.DataFrame) and _pending_id and _pending_id != "manual_entry":
        ...
        if _show_preview:
            render_table(_pending_preview_df.head(8))
...
def _render_data_input_workspace_body(lang: str) -> None:
    _value_mode = st.radio(
        tt("Değer yapısı", "Value structure"),
        options=["crisp", "tfn"],
        index=0 if str(st.session_state.get("data_value_mode", "crisp")) != "tfn" else 1,
        format_func=lambda mode: tt("🔹 Kesin sayılar", "🔹 Crisp values") if mode == "crisp" else tt("🔺 Üçgensel bulanık (TFN)", "🔺 Triangular fuzzy (TFN)"),
        horizontal=True,
        key="data_value_mode",
    )
    if _value_mode == "tfn":
        st.info(
            tt(
                "Fuzzy sistemler bu uygulamada üçgensel bulanık sayı (TFN) mantığıyla çalışır. Veri girişinde her kriter için `_l`, `_m`, `_u` sütunları beklenir.",
                "Fuzzy systems in this app use triangular fuzzy numbers (TFN). For data entry, each criterion is expected to have `_l`, `_m`, `_u` columns.",
            )
        )
    _mode = st.radio(
...
def _render_manual_entry_workspace(lang: str) -> None:
    ...
    with st.container():
        ...
        with _editor_col:
            ...
            for _crit_col in _criterion_input_columns(_manual_criteria, fuzzy_tfn=_is_tfn_input):
                _manual_col_cfg[_crit_col] = st.column_config.NumberColumn(_crit_col, format="%.4f")
            ...
            with _paste_tab:
                _expected_cols = _manual_schema_columns(_manual_entity_col, _manual_criteria, _manual_year_col, fuzzy_tfn=_is_tfn_input)
                if _is_tfn_input:
                    _sample_values: List[str] = []
                    for idx in range(len(_manual_criteria)):
                        mid = 10.0 + idx
                        _sample_values.extend([f"{mid-0.5:.2f}", f"{mid:.2f}", f"{mid+0.5:.2f}"])
                else:
                    _sample_values = [f"{10.0 + idx:.2f}" for idx in range(len(_manual_criteria))]
                _sample_row = ["A01"] + ([str(2024)] if _manual_year_col else []) + _sample_values
            ...
            _manual_stats = _manual_editor_stats(_manual_seed, _manual_entity_col, _manual_criteria, _manual_year_col, fuzzy_tfn=_is_tfn_input)
...
def _input_format_notes(lang: str, panel: bool = False, fuzzy_tfn: bool = False) -> pd.DataFrame:
    criterion_note = (
        _xl_text(
            lang,
    ...
    if panel:
        rows = [
            ...
            },
            {
                ...
                _xl_text(lang, "Açıklama", "Description"): criterion_note,
            },
        ]
    else:
        rows = [
            ...
            },
            {
                ...
                _xl_text(lang, "Açıklama", "Description"): _xl_text(lang, "Analiz için en az 2 kriter gerekir.", "At least 2 criteria are required."),
            },
            {
            ...
            {
                _xl_text(lang, "Başlık", "Item"): _xl_text(lang, "Kriter verisi", "Criterion data"),
                _xl_text(lang, "Açıklama", "Description"): criterion_note,
            },
        ]
    return pd.DataFrame(rows)

def generate_input_template_excel(lang: str = "TR", panel: bool = False, fuzzy_tfn: bool = False) -> bytes:
    output = io.BytesIO()
    if panel:
        sample = sample_panel_fuzzy_dataset_en().head(20) if fuzzy_tfn else sample_panel_dataset_en().head(20)
    else:
        if fuzzy_tfn:
            sample = sample_fuzzy_dataset_en() if lang == "EN" else sample_fuzzy_dataset()
        else:
            sample = sample_dataset_en() if lang == "EN" else sample_dataset()
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        ...
        _input_format_notes(lang, panel=panel, fuzzy_tfn=fuzzy_tfn).to_excel(writer, sheet_name=sheet_notes, index=False)
        ...
        for sheet_name, df in [(sheet_data, sample), (sheet_notes, _input_format_notes(lang, panel=panel, fuzzy_tfn=fuzzy_tfn))]:
...
def get_method_steps(method: str, stage: str) -> str:
    ...
    if is_en:
        weight_steps = {
            ...
            "Fuzzy Entropy": "1) Build lower/middle/upper fuzzy scenarios\n2) Compute entropy in each scenario\n3) Average scenario weights\n4) Normalize final fuzzy entropy weights",
            ...
            "Fuzzy CRITIC": "1) Build lower/middle/upper fuzzy scenarios\n2) Run CRITIC in each scenario\n3) Aggregate variance-conflict weights\n4) Normalize final fuzzy CRITIC weights",
            "Standart Sapma": "1) Normalize\n2) Compute criterion standard deviations\n3) Convert relative dispersion to weights",
            "Fuzzy Standart Sapma": "1) Build lower/middle/upper fuzzy scenarios\n2) Compute standard deviations in each scenario\n3) Average scenario dispersions\n4) Normalize final fuzzy SD weights",
            ...
            "Fuzzy MEREC": "1) Build lower/middle/upper fuzzy scenarios\n2) Compute removal effects in each scenario\n3) Average scenario importance vectors\n4) Normalize final fuzzy MEREC weights",
            "LOPCOW": "1) Normalize\n2) Compute RMS/std terms\n3) Logarithmic percentage variability\n4) Normalize weights",
            "Fuzzy LOPCOW": "1) Build lower/middle/upper fuzzy scenarios\n2) Compute RMS/std terms per scenario\n3) Aggregate log percentage variability\n4) Normalize final fuzzy LOPCOW weights",
            "PCA": "1) Standardize\n2) Extract principal components\n3) Combine loadings and explained variance\n4) Produce criterion weights",
            "Fuzzy PCA": "1) Build lower/middle/upper fuzzy scenarios\n2) Run PCA in each scenario\n3) Average component-based criterion weights\n4) Normalize final fuzzy PCA weights",
            ...
            "Fuzzy CILOS": "1) Build lower/middle/upper fuzzy scenarios\n2) Compute CILOS impact-loss weights in each scenario\n3) Average scenario weights\n4) Normalize final fuzzy CILOS weights",
            "SPC": "1) Min-max normalization\n2) Compute symmetry points\n3) Measure total deviations\n4) Normalize SPC weights",
            "Fuzzy SPC": "1) Build lower/middle/upper fuzzy scenarios\n2) Compute symmetry-point deviations in each scenario\n3) Average scenario distances\n4) Normalize final fuzzy SPC weights",
            "AHP": "1) Build pairwise comparison matrix\n2) Extract principal eigenvector\n3) Compute CR consistency ratio\n4) Normalize final weights",
            "Fuzzy AHP": "1) Build lower/middle/upper pairwise scenarios\n2) Run AHP in each scenario\n3) Aggregate scenario weights and CR\n4) Normalize final fuzzy AHP weights",
            "BWM": "1) Define best and worst criteria\n2) Build best-to-others and others-to-worst vectors\n3) Solve linear model\n4) Validate xi/CR and normalize weights",
            "Fuzzy BWM": "1) Build lower/middle/upper best/worst scenarios\n2) Solve BWM in each scenario\n3) Aggregate scenario xi and weights\n4) Normalize final fuzzy BWM weights",
            "SWARA": "1) Rank criteria by importance\n2) Enter stepwise relative importance reductions\n3) Compute k and q coefficients\n4) Normalize SWARA weights",
            "Fuzzy SWARA": "1) Build lower/middle/upper SWARA reductions\n2) Compute scenario q coefficients\n3) Average scenario weights\n4) Normalize final fuzzy SWARA weights",
            "DEMATEL": "1) Normalize direct relation matrix\n2) Compute total relation matrix\n3) Derive prominence and relation\n4) Normalize criterion weights",
            "Fuzzy DEMATEL": "1) Build lower/middle/upper relation scenarios\n2) Run DEMATEL in each scenario\n3) Aggregate prominence-relation outputs\n4) Normalize final fuzzy DEMATEL weights",
            "SMART": "1) Collect direct criterion scores\n2) Convert points to ratios\n3) Normalize weights\n4) Proceed to ranking",
            "Fuzzy SMART": "1) Build lower/middle/upper score scenarios\n2) Run SMART in each scenario\n3) Average scenario weights\n4) Normalize final fuzzy SMART weights",
        }
        ranking_steps = {
    ...
    weight_steps = {
        ...
        "Fuzzy Entropy": "1) Alt/orta/üst bulanık senaryoları kur\n2) Her senaryoda entropi hesapla\n3) Senaryo ağırlıklarını ortala\n4) Nihai bulanık entropi ağırlıklarını normalize et",
        "CRITIC": "1) Min-max normalize et\n2) Standart sapmaları bul\n3) Korelasyon yapısını çıkar\n4) Bilgi içeriğinden ağırlık üret",
        "Fuzzy CRITIC": "1) Alt/orta/üst bulanık senaryoları kur\n2) Her senaryoda CRITIC çalıştır\n3) Varyans-çatışma ağırlıklarını birleştir\n4) Nihai bulanık CRITIC ağırlıklarını normalize et",
        "Standart Sapma": "1) Normalize et\n2) Kriter std sapmalarını hesapla\n3) Std oranlarını ağırlığa çevir",
        "Fuzzy Standart Sapma": "1) Alt/orta/üst bulanık senaryoları kur\n2) Her senaryoda std sapmaları hesapla\n3) Senaryo yayılımlarını ortala\n4) Nihai bulanık std ağırlıklarını normalize et",
        ...
        "Fuzzy MEREC": "1) Alt/orta/üst bulanık senaryoları kur\n2) Her senaryoda çıkarma etkisini hesapla\n3) Senaryo önem vektörlerini ortala\n4) Nihai bulanık MEREC ağırlıklarını normalize et",
        "LOPCOW": "1) Normalize et\n2) RMS ve std oranlarını hesapla\n3) Logaritmik yüzde değişim gücü\n4) Ağırlıkları normalize et",
        "Fuzzy LOPCOW": "1) Alt/orta/üst bulanık senaryoları kur\n2) Her senaryoda RMS ve std oranlarını hesapla\n3) Logaritmik değişim gücünü birleştir\n4) Nihai bulanık LOPCOW ağırlıklarını normalize et",
        "PCA": "1) Standardize et\n2) Temel bileşenleri çıkar\n3) Yükleri ve açıklanan varyansı birleştir\n4) Kriter ağırlıklarını üret",
        "Fuzzy PCA": "1) Alt/orta/üst bulanık senaryoları kur\n2) Her senaryoda PCA çalıştır\n3) Bileşen tabanlı kriter ağırlıklarını ortala\n4) Nihai bulanık PCA ağırlıklarını normalize et",
        ...
        "Fuzzy CILOS": "1) Alt/orta/üst bulanık senaryoları kur\n2) Her senaryoda CILOS kayıp etkisini hesapla\n3) Senaryo ağırlıklarını ortala\n4) Nihai bulanık CILOS ağırlıklarını normalize et",
        "SPC": "1) Min-max normalize et\n2) Simetri noktalarını bul\n3) Toplam sapmaları hesapla\n4) SPC ağırlıklarını normalize et",
        "Fuzzy SPC": "1) Alt/orta/üst bulanık senaryoları kur\n2) Her senaryoda simetri noktası sapmalarını hesapla\n3) Senaryo uzaklıklarını ortala\n4) Nihai bulanık SPC ağırlıklarını normalize et",
        "AHP": "1) İkili karşılaştırma matrisi kur\n2) Özvektörü çıkar\n3) CR tutarlılık oranını hesapla\n4) Nihai ağırlıkları normalize et",
        "Fuzzy AHP": "1) Alt/orta/üst ikili karşılaştırma senaryoları kur\n2) Her senaryoda AHP çalıştır\n3) Senaryo ağırlıklarını ve CR değerlerini birleştir\n4) Nihai bulanık AHP ağırlıklarını normalize et",
        "BWM": "1) En iyi ve en kötü kriteri belirle\n2) Best-to-others ve others-to-worst vektörlerini kur\n3) Doğrusal modeli çöz\n4) Xi/CR ile tutarlılığı doğrula ve ağırlıkları normalize et",
        "Fuzzy BWM": "1) Alt/orta/üst en iyi-en kötü senaryoları kur\n2) Her senaryoda BWM çöz\n3) Senaryo xi ve ağırlıklarını birleştir\n4) Nihai bulanık BWM ağırlıklarını normalize et",
        "SWARA": "1) Kriterleri önem sırasına koy\n2) Adım adım göreli önem azaltımlarını gir\n3) k ve q katsayılarını hesapla\n4) SWARA ağırlıklarını normalize et",
        "Fuzzy SWARA": "1) Alt/orta/üst SWARA azaltım senaryolarını kur\n2) Her senaryoda q katsayılarını hesapla\n3) Senaryo ağırlıklarını ortala\n4) Nihai bulanık SWARA ağırlıklarını normalize et",
        "DEMATEL": "1) Doğrudan ilişki matrisini normalize et\n2) Toplam ilişki matrisini hesapla\n3) Prominence ve relation değerlerini çıkar\n4) Kriter ağırlıklarını normalize et",
        "Fuzzy DEMATEL": "1) Alt/orta/üst ilişki senaryolarını kur\n2) Her senaryoda DEMATEL çalıştır\n3) Merkezilik ve etki-sonuç çıktısını birleştir\n4) Nihai bulanık DEMATEL ağırlıklarını normalize et",
        "SMART": "1) Doğrudan kriter puanlarını topla\n2) Puanları oranlara çevir\n3) Ağırlıkları normalize et\n4) Sıralama aşamasına geç",
        "Fuzzy SMART": "1) Alt/orta/üst puan senaryolarını kur\n2) Her senaryoda SMART çalıştır\n3) Senaryo ağırlıklarını ortala\n4) Nihai bulanık SMART ağırlıklarını normalize et",
    }
    ranking_steps = {
...
with st.sidebar:
    ...
    if not _current_user.is_logged_in:
        ...
        with st.expander(tt("📘 Uygulama Rehberi", "📘 User Guide"), expanded=True):
            _guest_guide = tt(
                "1️⃣ Giriş yap.<br>2️⃣ Veri yükle (Excel, CSV, SPSS).<br>3️⃣ Yöntemini seç.<br>4️⃣ Analiz et ve raporu indir.",
        ...
        with st.expander(tt("🧭 Neden MCDM Karar Destek Sistemi?", "🧭 Why MCDM Karar Destek Sistemi?"), expanded=False):
            _guest_why = tt(
                "✅ 24+ klasik ve bulanık sıralama yöntemi<br>✅ 9 objektif ağırlık yöntemi<br>✅ Monte Carlo duyarlılık analizi<br>✅ SSCI akademik rapor (Word + Excel)<br>✅ Türkçe ve İngilizce arayüz",
        ...
        with st.expander(tt("🧭 Metodolojik Yardım", "🧭 Methodological Help"), expanded=False):
            _method_help = tt(
                "<b>Objektif Ağırlık Yöntemleri:</b> Kriter önemini verinin kendi yapısından türetir; araştırmacı müdahalesi gerekmez.<br><br>"
    ...
    with st.container():
        ...
        with st.container():
            ...
            if "Objektif" in weight_mode or "Objective" in weight_mode:
                ...
                weight_method = st.session_state.get("subjective_method_name", "Subjektif (Uzman) Ağırlık")
                ...
                if _sub_weights:
                    weight_method = st.session_state.get("subjective_method_name", weight_method)
                    st.session_state["weight_method_pref"] = weight_method
    ...
    if _wr.get("info"):
        ...
        with st.expander(f"📋 {tt('Tablolar — Detay Sağlamlık Verileri', 'Tables — Detailed Robustness Data')}", expanded=False):
            ...
            if isinstance(_loo_df, pd.DataFrame) and not _loo_df.empty:
                ...
                with st.container():
                    _loo_comment = tt(
                        "Leave-one-out tablosu, her alternatifin çıkarıldığında ağırlık düzeninin ne ölçüde korunduğunu gösterir. SpearmanRho sütunu 1'e yakın değerler yüksek kararlılık anlamına gelir. MaksMutlakFark ise en fazla değişen kriter ağırlığının büyüklüğünü verir.",
                        "The leave-one-out table shows how well the weight order is preserved when each alternative is removed. SpearmanRho values close to 1 indicate high stability. MaxAbsoluteDiff gives the magnitude of the most changed criterion weight.",
                    )
                    st.markdown(f'<div class="commentary-box">{_loo_comment}</div>', unsafe_allow_html=True)
        ...
        if ranking_table is None or ranking_table.empty:
            ...
            with st.expander(f"📊 {tt('Şekiller', 'Figures')}", expanded=True):
                ...
                if isinstance(contrib, pd.DataFrame):
                    ...
                    with st.container():
                        _parallel_comment = tt(
                            "Paralel koordinat grafiği, her alternatifin tüm kriterler boyunca performans profilini tek bir görünümde sunar. Çizgilerin rengi ve yoğunluğu skor büyüklüğünü gösterir. Çizgilerin yoğun kesiştiği bölgeler, alternatiflerin benzer kriter değerlerine sahip olduğu alanları işaret eder.",
                            "The parallel coordinates chart presents each alternative's performance profile across all criteria in a single view. Line color and intensity show score magnitude. Regions where lines densely intersect indicate areas where alternatives have similar criterion values.",
                        )
                        st.markdown(f'<div class="commentary-box">{_parallel_comment}</div>', unsafe_allow_html=True)
            ...
            if isinstance(mc_df, pd.DataFrame) and not mc_df.empty:
                ...
                with st.expander(f"📊 {tt('Şekiller', 'Figures')}", expanded=True):
                    ...
                    with mg1:
                        ...
                        with st.container():
                            _rank_bar_comment = tt(
                                "Birincilik oranı çubuğu, her alternatifin Monte Carlo simülasyonları boyunca kaç kez birinci sıraya oturduğunu yüzde olarak gösterir. %50 üzeri oran güçlü kararlılığı ifade eder; %20 altı oran alternatifin liderliğinin rastlantısal olduğuna işaret edebilir.",
                                "The first-place rate bar shows what percentage of Monte Carlo simulations each alternative ranked first. A rate above 50% indicates strong stability; a rate below 20% may indicate the alternative's leadership is coincidental.",
                            )
                            st.markdown(f'<div class="commentary-box">{_rank_bar_comment}</div>', unsafe_allow_html=True)
                    with mg2:
                    ...
                    if isinstance(loc_df, pd.DataFrame) and not loc_df.empty:
                        ...
                        with st.container():
                            _local_comment = tt(
                                "Lokal duyarlılık grafiği, her kriterin ağırlığı ±%10 ve ±%20 değiştirildiğinde sıralamanın ne kadar etkilendiğini Spearman korelasyonu ile ölçer. ρ = 1.0'a yakın çizgiler, o kriterin ağırlık değişimine karşı sıralamanın kararlı kaldığını gösterir; aşağı düşen çizgiler kritik duyarlılık noktalarına işaret eder.",
                                "The local sensitivity chart measures how much the ranking is affected when each criterion weight changes by ±10% and ±20%, using Spearman correlation. Lines close to ρ = 1.0 show the ranking remains stable against that criterion weight change; lines dropping down indicate critical sensitivity points.",
                            )
                            st.markdown(f'<div class="commentary-box">{_local_comment}</div>', unsafe_allow_html=True)
