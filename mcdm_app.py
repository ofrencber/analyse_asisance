...
def _activate_data_source(df: pd.DataFrame, source_id: str, entry_mode: str) -> None:
    ...
    # TFN sütunları varsa otomatik TFN moduna geç
    if value_mode != "tfn" and _detect_tfn_groups(list(df.columns)):
        value_mode = "tfn"
            ...
            if _data_has_tfn:
                st.caption(
                    tt(
                        ...
                        "This preview shows the raw TFN columns (`_l`, `_m`, `_u`) exactly as uploaded.",
                    )
                )
            render_table(_pending_preview_df.head(8))
            if _pending_crisp_preview_df is not None:
                _show_centroid_preview = st.checkbox(
                    tt("🧮 Ağırlık merkezi (crisp) görünümünü de göster", "🧮 Also show centroid (crisp) view"),
                    key="chk_preview_upload_centroid",
                )
                if _show_centroid_preview:
                    render_table(_pending_crisp_preview_df.head(8))
