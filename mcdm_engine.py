...
METHOD_PHILOSOPHY: Dict[str, Dict[str, str]] = {
    ...
    "Fuzzy Entropy": {
        "simple": "Fuzzy Entropy, bilgi çeşitliliğini belirsizlik altında alt-orta-üst senaryolar üzerinden ölçer.",
        "academic": "Fuzzy Entropy, Shannon entropisini üçgensel bulanık senaryolar altında çalıştırarak kriterlerin belirsizlikte de ayırt edicilik gücünü nesnel biçimde nicelleştirir.",
    },
    "CRITIC": {
    ...
    "Fuzzy CRITIC": {
        "simple": "Fuzzy CRITIC, varyans ve kriter çatışmasını belirsizlik altında birlikte değerlendirir.",
        "academic": "Fuzzy CRITIC, CRITIC yönteminin standart sapma ve korelasyon çatışması mantığını alt-orta-üst bulanık veri senaryolarında çalıştırarak belirsizlik duyarlı objektif ağırlıklar üretir.",
    },
    "Standart Sapma": {
    ...
    "Fuzzy Standart Sapma": {
        "simple": "Fuzzy standart sapma, kriterlerin ayırt ediciliğini belirsizlik altında dağılım genişliğiyle ölçer.",
        "academic": "Fuzzy standart sapma ağırlıklandırması, min-max normalize edilen alt-orta-üst bulanık senaryolardaki yayılım genişliğini toplulaştırarak kriter önemini hesaplar.",
    },
    "MEREC": {
    ...
    "Fuzzy MEREC": {
        "simple": "Fuzzy MEREC, kriter çıkarma etkisini belirsizlik altında senaryo bazlı olarak ölçer.",
        "academic": "Fuzzy MEREC, MEREC'in çıkarma etkisi mantığını bulanıklaştırılmış alt-orta-üst performans senaryolarında uygulayarak kriterlerin sistemik zorunluluğunu belirsizlik altında değerlendirir.",
    },
    "LOPCOW": {
    ...
    "Fuzzy LOPCOW": {
        "simple": "Fuzzy LOPCOW, logaritmik yüzde değişim gücünü belirsizlik altında üç senaryoda hesaplar.",
        "academic": "Fuzzy LOPCOW, kriterlerin normalize edilmiş RMS/standart sapma yapısını bulanık alt-orta-üst senaryolarda inceleyerek ölçek farklarına dayanıklı ve belirsizlik duyarlı ağırlıklar üretir.",
    },
    "PCA": {
    ...
    "Fuzzy PCA": {
        "simple": "Fuzzy PCA, korelasyon yapısını belirsizlik altında temel bileşenler üzerinden yorumlar.",
        "academic": "Fuzzy PCA, temel bileşen ağırlıklarını bulanıklaştırılmış alt-orta-üst veri senaryolarında hesaplayıp ortak varyans yapısını belirsizlik altında birleştirerek kriter ağırlığı üretir.",
    },
    "IDOCRIW": {
    ...
    "Fuzzy SPC": {
        "simple": "Fuzzy SPC, kriterlerin simetri noktasından sapmasını belirsizlik altında ölçer.",
        "academic": "Fuzzy SPC, SPC'nin simetri noktasına uzaklık mantığını alt-orta-üst bulanık senaryolarda hesaplayarak kriter ayırt ediciliğini belirsizlik altında mesafe temelli biçimde değerlendirir.",
    },
    "SPOTIS": {
    ...
    "AHP": {
        "simple": "AHP, kriterleri ikili karşılaştırmalarla önceliklendirir ve yanıtların tutarlılığını ölçer.",
        "academic": "AHP, özdeğer yaklaşımıyla ikili karşılaştırma matrisinden ağırlık türeten ve Tutarlılık Oranı (CR) ile uzman yargısının iç mantığını denetleyen klasik bir subjektif ağırlıklandırma yöntemidir.",
    },
    "Fuzzy AHP": {
        "simple": "Fuzzy AHP, ikili karşılaştırmaları belirsizlik altında bulanık önceliklerle değerlendirir.",
        "academic": "Fuzzy AHP, AHP'nin ikili karşılaştırma mantığını üçgensel bulanık tercihler ve senaryo tabanlı özdeğer çözümü üzerinden genişleterek uzman kararsızlığını modele taşır.",
    },
    "BWM": {
        "simple": "BWM, en iyi ve en kötü kriteri referans alarak az sayıda karşılaştırmayla ağırlık üretir.",
        "academic": "BWM, en iyi ve en kötü kriter referansları üzerinden doğrusal optimizasyonla ağırlık türeten ve xi/CR göstergeleriyle tutarlılığı denetleyen parsimonik bir subjektif ağırlıklandırma yöntemidir.",
    },
    "Fuzzy BWM": {
        "simple": "Fuzzy BWM, en iyi-en kötü referanslarını belirsizlik altında bulanık üstünlüklerle yorumlar.",
        "academic": "Fuzzy BWM, BWM'nin doğrusal modelini alt-orta-üst bulanık üstünlük senaryolarına genişleterek az soru ile belirsizlik duyarlı uzman ağırlıkları üretir.",
    },
    "SWARA": {
        "simple": "SWARA, kriterleri önem sırasına koyup ardışık göreli önem azaltımlarıyla ağırlıklandırır.",
        "academic": "SWARA, uzmanların önceden sıralanmış kriterler için göreli önem azaltım katsayılarını kullanarak adım adım ağırlık türeten sezgisel ve düşük bilişsel yüklü bir subjektif yöntemdir.",
    },
    "Fuzzy SWARA": {
        "simple": "Fuzzy SWARA, ardışık önem azaltımlarını belirsizlik altında bulanık senaryolarla yorumlar.",
        "academic": "Fuzzy SWARA, SWARA'nın adım adım göreli önem azaltımı mantığını bulanık alt-orta-üst senaryolar üzerinde işleyerek uzman kararsızlığını kademeli ağırlık vektörüne taşır.",
    },
    "DEMATEL": {
        "simple": "DEMATEL, kriterlerin birbirini ne ölçüde etkilediğini kullanarak ağırlık ve neden-sonuç yapısı üretir.",
        "academic": "DEMATEL, doğrudan ilişki matrisini normalize edip toplam etki matrisine dönüştürerek kriterlerin prominence ve relation değerlerini çıkaran ağ-temelli bir subjektif ağırlıklandırma yaklaşımıdır.",
    },
    "Fuzzy DEMATEL": {
        "simple": "Fuzzy DEMATEL, kriter etkileşimlerini belirsizlik altında bulanık etki senaryolarıyla değerlendirir.",
        "academic": "Fuzzy DEMATEL, DEMATEL'in toplam etki matrisi mantığını alt-orta-üst bulanık ilişki senaryolarında uygulayarak merkezilik ve neden-sonuç yapısını belirsizlik duyarlı biçimde modeller.",
    },
    "SMART": {
        "simple": "SMART, kriterlere doğrudan önem puanı vererek hızlı ve anlaşılır ağırlık üretir.",
        "academic": "SMART, karar vericinin doğrudan puanlarını normalize ederek ağırlık vektörüne çeviren, öğrenmesi kolay ve iletişimi güçlü bir subjektif ağırlıklandırma tekniğidir.",
    },
    "Fuzzy SMART": {
        "simple": "Fuzzy SMART, doğrudan uzman puanlarını belirsizlik altında bulanık önem bantlarıyla işler.",
        "academic": "Fuzzy SMART, SMART'ın doğrudan puanlama yaklaşımını alt-orta-üst bulanık senaryolar üzerinden yorumlayarak uzman tereddüdünü normalize edilmiş ağırlıklara yansıtır.",
    },
}
...
def _build_fuzzy_data_scenarios(data: pd.DataFrame, spread: float = 0.10) -> List[Tuple[str, pd.DataFrame]]:
    base = data.copy().astype(float)
    delta = base.abs() * max(float(spread), 0.0)
    lower = base - delta
    middle = base.copy()
    upper = base + delta
    ...
    *,
    ...
    for scenario_name, scenario_df in scenarios:
        ...
        for crit, val in zip(cols, vec):
            rows.append({"Senaryo": scenario_name, "Kriter": crit, "Ağırlık": val})
        for key, value in (det or {}).items():
    ...
    det: Dict[str, Any] = {
        ...
        detail_key: pd.DataFrame({"Kriter": cols, "OrtalamaAğırlık": avg, "Ağırlık": weights}),
        "parameters": {"spread": float(spread)},
    }
    for key, frames in detail_frames.items():
        det[f"fuzzy_{key}"] = pd.concat(frames, ignore_index=True)
    return w, det
...
def generate_3layer_weight(
    ...
    subjective_methods = {"AHP", "Fuzzy AHP", "BWM", "Fuzzy BWM", "SWARA", "Fuzzy SWARA", "DEMATEL", "Fuzzy DEMATEL", "SMART", "Fuzzy SMART"}
    base_method = weight_method.replace("Fuzzy ", "") if weight_method.startswith("Fuzzy ") else weight_method

    if weight_method.startswith("Fuzzy ") and weight_method not in descriptive:
        descriptive_text = (
            f"{weight_method} yöntemi belirsizlik altında alt-orta-üst senaryoları toplulaştırarak "
            f"**{top_crit}** kriterini en baskın unsur (w = {top_w:.4f}, %{top_w*100:.1f}) olarak belirledi. "
            f"İkinci sıradaki **{second_crit}** ile yoğunlaşma oranı {concentration:.2f}x düzeyindedir."
        )
        analytic_text = analytic.get(
            weight_method,
            METHOD_PHILOSOPHY.get(weight_method, {}).get("academic", METHOD_PHILOSOPHY.get(base_method, {}).get("academic", "")),
        )
        normative_text = (
            f"Belirsizlik altında da baskın kalan **{top_crit}** kriteri kararın ana sürükleyicisi olarak izlenmelidir. "
            f"Ağırlık farkı sınırlıysa **{second_crit}** kriteriyle birlikte yakın-duyarlılık analizi yapılması önerilir."
        )
    elif weight_method in subjective_methods and weight_method not in descriptive:
        descriptive_text = (
            f"{weight_method} uzman yargılarına dayalı ağırlıklandırmada **{top_crit}** kriterini "
            f"en yüksek öncelik olarak öne çıkardı (w = {top_w:.4f}, %{top_w*100:.1f}). "
            f"**{bottom_crit}** ise en düşük uzman önceliğini aldı."
        )
        analytic_text = analytic.get(
            weight_method,
            METHOD_PHILOSOPHY.get(weight_method, {}).get("academic", ""),
        )
        normative_text = (
            f"Uzman öncelik yapısına göre **{top_crit}** kriteri saha kararlarında ve rapor yorumunda ana referans alınmalıdır. "
            f"Yine de subjektif yapı nedeniyle sonuçlar duyarlılık analizi ile birlikte okunmalıdır."
        )
    else:
        descriptive_text = descriptive.get(weight_method, f"En yüksek ağırlık **{top_crit}** kriterine atandı (w = {top_w:.4f}).")
        analytic_text = analytic.get(weight_method, METHOD_PHILOSOPHY.get(weight_method, {}).get("academic", ""))
        normative_text = normative.get(weight_method, f"Karar vericiler **{top_crit}** kriterini öncelikli değerlendirmelidir.")

    return {
        "descriptive": descriptive_text,
        "analytic": analytic_text,
        "normative": normative_text,
    }
